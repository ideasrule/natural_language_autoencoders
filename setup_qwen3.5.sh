#!/bin/bash
# =============================================================================
# Set up a fresh machine to train the Qwen3.5-9B NLA (companion to
# train_qwen3.5.sh — run this once, then that).
#
# Produces the exact environment the Qwen3.5-9B run was built against:
...
#     local CUDA 13 toolkit — Qwen3.5's gated-deltanet layers need both)
#   * this repo (nla) editable, Qwen/Qwen3.5-9B in the HF cache
#
...
#   * ~80GB free under $WORKSPACE (checkouts + HF model + wheels)
...
# .nla_meta.yaml sidecars (Qwen3.5-9B layer 21) must already be at $DATA.
# =============================================================================
set -euo pipefail
...
echo "=== [6/8] linear-attention kernels (Qwen3.5 gated-deltanet layers) ==="
...
for f in "$DATA"/ar_sft_shuf.parquet "$DATA"/av_sft_shuf.parquet "$DATA"/rl_shuf.parquet; do
    [ -f "$f" ] && [ -f "$f.nla_meta.yaml" ] || echo "WARNING: missing $f (+sidecar) — stage the training data before running train_qwen3.5.sh"
done

echo "=== setup complete — next: bash $NLA_REPO/train_qwen3.5.sh ==="
